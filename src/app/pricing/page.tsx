...
const Pricing = () => {
    ...
    const handlePlanUpdate = async (plan: Plan) => {
        if (!session?.user) {
            toast("Sign in to get Pro", {
                action: {
                    label: "Sign in",
        ...
        if (session.user.plan === plan) {
            toast("You already have the Pro plan");
            return;
        }
        try {
            setLoading(true);
                ...
                <div className="grid lg:grid-cols-2 gap-8 max-w-4xl mx-auto">
                    {plans?.map((plan, index) => (
                        ...
                        >
                            ...
                            <div
                                className={`h-full glass rounded-2xl p-8 border transition-all duration-300 ${plan.popular
                            ...
                            >
                                ...
                                >
                                    {plan.cta === "Pro" && loading ? "Getting pro..." : plan.cta}
                                </Button>
                            </div>
                        </motion.div>
                    ))}
